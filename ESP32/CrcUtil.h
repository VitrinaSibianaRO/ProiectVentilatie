...
// Formateaza uint16_t ca 4 hex uppercase in buf[5].
...
// Parseaza 4 hex chars (case-insensitive) in uint16_t. Returneaza false la char invalid.
...
// Validare end-to-end: extrage CRC din buf, verifica match cu payload.
// buf NULL-terminated, format: "<payload>*<crc4hex>" (fara \n).
// Returneaza true daca CRC valid; false la lipsa '*', format gresit sau mismatch.
...
// Strip "*XXXX" din buffer in-place — taie la asterisc.
inline void stripCrc(char* buf) {
    char* asterisk = strrchr(buf, '*');
    if (asterisk) *asterisk = '\0';
}

// Append CRC la un string in buffer de out_size bytes.
// Returneaza false daca nu incape. Buffer-ul trebuie sa aiba minim strlen(payload)+5 bytes.
inline bool appendCrc(char* buf, size_t out_size) {
    size_t payloadLen = strlen(buf);
    if (payloadLen + 5 > out_size) return false;  // '*' + 4 hex + '\0'
