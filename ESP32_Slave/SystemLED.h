...
public:
  ...
  SystemLED(uint8_t dataPin, uint8_t enablePin, uint8_t count)
      : _strip(count, dataPin, NEO_GRB + NEO_KHZ800), _enablePin(enablePin),
        _status(Status::Off) {}

  void begin() {
    pinMode(_enablePin, OUTPUT);
    digitalWrite(_enablePin,
                 LOW); // LOW activeaza alimentarea LED-ului pe placa Carbon S2
    _strip.begin();
...
private:
  ...
  static uint32_t _statusToColor(Status s) {
    switch (s) {
    case Status::Booting:
      return 0xFF8000; // oranj
    case Status::Ready:
      return 0x00E0FF; // cyan
    case Status::Active:
